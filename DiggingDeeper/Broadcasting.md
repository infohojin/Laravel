...
이제, 여러분이 평소와 동일하게 이벤트를 발생시키기만 하면 됩니다. 이벤트가 한 번 발생하고나면, 큐-queue에 추가된 job)이 자동으로 여러분이 지정한 브로드캐스트 드라이버를 통해 이벤트를 자동으로 전달 할 것입니다.
...
pusher 커넥터를 사용하는 에코 인스턴스를 만들 때에는, cluster와 커넥션이 암호화 되어야 하는지 여부도 명시할 수 있습니다.
